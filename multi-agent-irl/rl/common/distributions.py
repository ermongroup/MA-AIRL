...
class CategoricalPdOneHotType(PdType):
    def __init__(self, ncat):
        self.ncat = ncat
    def pdclass(self):
        return CategoricalPdOneHot
    def param_shape(self):
        return [self.ncat]
    def sample_shape(self):
        return [self.ncat]
...
class CategoricalPdOneHot(Pd):
    def __init__(self, logits):
        self.logits = logits
    def flatparam(self):
        return self.logits
    def mode(self):
        return tf.one_hot(U.argmax(self.logits, axis=-1), self.logits.get_shape().as_list()[-1])
    def neglogp(self, x):
        # return tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.logits, labels=x)
        # Note: we can't use sparse_softmax_cross_entropy_with_logits because
        #       the implementation does not allow second-order derivatives...
    ...
    def sample(self):
        u = tf.random_uniform(tf.shape(self.logits))
        return tf.one_hot(tf.argmax(self.logits - tf.log(-tf.log(u)), axis=-1),
